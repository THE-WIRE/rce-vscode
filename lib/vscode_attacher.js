...
function shareVscodeEditor(editor , ctx){ // editor not covered
    ...
    function applyToSharejs(event){
        ...
        //changes.forEach(change => {
            ...
            //TODO: add columns in offset (test remaining)


            if(event.contentChanges.length > 0){
                var loop = event.contentChanges[0].range
                console.log(event.contentChanges[0].range)
            
                for(var a in loop){
                     var line  = loop[a].line;
                     var char  = loop[a].character;
                     break; 
                 }
                 console.log('line : '+line);
                 console.log('char : '+ char);
                pos = vscode.window.activeTextEditor.document.offsetAt(new vscode.Position(line,char)); 
