...
function shareVscodeEditor(editor , ctx){ // editor not covered
    ...
    function applyToSharejs(event){
        ...
        //changes.forEach(change => {
            ...
            //TODO: add columns in offset (test remaining) // ?????????? may be it is resolved
            if(event.contentChanges[0]){
                // position is changed taking api contenchanges in reference 
                pos = vscode.window.activeTextEditor.document.offsetAt(new vscode.Position(event.contentChanges[0].range[0].line , event.contentChanges[0].range[0].character));
                // pos = [change.startLineNumber, change.startColumn]; 
                console.log('this is a position : '+pos);
                if(pos){
